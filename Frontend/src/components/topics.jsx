import React, { useLayoutEffect, useRef } from "react";
import { Link } from "react-router-dom";
import { Pindata } from "../context/pincontext";
import logo from "../assets/S-o.png";
import gsap from "gsap";

const Topics = () => {
  const { filter, setfilter } = Pindata();

  const bgRefDesktop = useRef(null);
  const buttonRefsDesktop = useRef([]);

  const bgRefMobile = useRef(null);
  const buttonRefsMobile = useRef([]);

  // 🔥 FIX: useLayoutEffect + gsap.set
  useLayoutEffect(() => {
    const isMobile = window.innerWidth < 768;
    const bg = isMobile ? bgRefMobile.current : bgRefDesktop.current;
    const buttons = isMobile
      ? buttonRefsMobile.current
      : buttonRefsDesktop.current;

    const activeBtn = buttons.find(
      (btn) => btn?.dataset.filter === filter
    );

    if (!activeBtn || !bg) return;

    // 👇 Instantly position (no animation)
    gsap.set(bg, {
      x: activeBtn.offsetLeft,
      width: activeBtn.offsetWidth,
      opacity: 1,
    });
  }, []); // 👈 only on mount

  // Animate on filter change
  useLayoutEffect(() => {
    const isMobile = window.innerWidth < 768;
    const bg = isMobile ? bgRefMobile.current : bgRefDesktop.current;
    const buttons = isMobile
      ? buttonRefsMobile.current
      : buttonRefsDesktop.current;

    const activeBtn = buttons.find(
      (btn) => btn?.dataset.filter === filter
    );

    if (!activeBtn || !bg) return;

    gsap.to(bg, {
      x: activeBtn.offsetLeft,
      width: activeBtn.offsetWidth,
      duration: 0.35,
      ease: "power3.out",
    });
  }, [filter]);

  return (
    <>
      {/* Desktop */}
      <div className="hidden md:block">
        <nav className="h-14 w-full bg-black/90 backdrop-blur-xl rounded-b-xl">
          <div className="flex justify-center mt-3 relative">
            <div className="relative flex gap-2">
              <span
                ref={bgRefDesktop}
                className="absolute top-0 left-0 p-4 bg-blue-600 rounded-2xl z-0"
                style={{ opacity: 0 }}
              />

              <button
                ref={(el) => (buttonRefsDesktop.current[0] = el)}
                data-filter="all"
                className="relative z-10 text-white px-3 py-1"
                onClick={() => setfilter("all")}
              >
                For you
              </button>

              <button
                ref={(el) => (buttonRefsDesktop.current[1] = el)}
                data-filter="following"
                className="relative z-10 text-white px-3 py-1"
                onClick={() => setfilter("following")}
              >
                Following
              </button>
            </div>
          </div>
        </nav>
      </div>

      {/* Mobile */}
      <div className="block md:hidden">
        <nav className="h-16 fixed top-0 w-full bg-black/90 backdrop-blur-xl z-50 rounded-b-2xl">
          <div className="relative flex justify-center items-center mt-4">
            <Link to="/" className="absolute left-4">
              <img src={logo} className="h-8" />
            </Link>

            <div className="relative flex gap-2">
              <span
                ref={bgRefMobile}
                className="absolute top-0 left-0 p-4 bg-blue-500 rounded-2xl z-0"
                style={{ opacity: 0 }}
              />

              <button
                ref={(el) => (buttonRefsMobile.current[0] = el)}
                data-filter="all"
                className="relative z-10 text-white px-3 py-1"
                onClick={() => setfilter("all")}
              >
                For you
              </button>

              <button
                ref={(el) => (buttonRefsMobile.current[1] = el)}
                data-filter="following"
                className="relative z-10 text-white px-3 py-1"
                onClick={() => setfilter("following")}
              >
                Following
              </button>
            </div>
          </div>
        </nav>
      </div>
    </>
  );
};

export default Topics;
